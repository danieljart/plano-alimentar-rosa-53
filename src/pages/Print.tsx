...
type Meal = { nome: string; itens: FoodItem[]; calorias: number; image: string };
...
function genDay(cal: number, gosta: string[], priorizaFrango: boolean): DayPlan {
  ...
  const p1 = priorizaFrango ? frango : pick(prot);
  const p2 = priorizaFrango ? frango : pick(prot);
  const meals: Meal[] = [
    { nome: "Café", itens: [pick(carb), pick(frutas), pick(lact)], calorias: Math.round(cal*dist["Café"]), image: by("Carboidratos")[0]?.image_url || "" },
    { nome: "Colação", itens: [pick(snack), pick(frutas)], calorias: Math.round(cal*dist["Colação"]), image: pick(snack).image_url },
    { nome: "Almoço", itens: [p1, pick(carb), pick(legu), pick(vegs)], calorias: Math.round(cal*dist["Almoço"]), image: p1.image_url },
    { nome: "Lanche", itens: [pick(carb), pick(frutas)], calorias: Math.round(cal*dist["Lanche"]), image: pick(carb).image_url },
    { nome: "Jantar", itens: [p2, pick(carb), pick(vegs)], calorias: Math.round(cal*dist["Jantar"]), image: p2.image_url },
    { nome: "Ceia", itens: [pick(snack)], calorias: Math.round(cal*dist["Ceia"]), image: pick(snack).image_url },
  ];
...
export default function PrintPage() {
  ...
  const plan: DayPlan | null = useMemo(() => {
    ...
    return genDay(prefs.caloriasDiarias, prefs.gostaIds, prefs.priorizarFrango);
  }, [prefs]);
