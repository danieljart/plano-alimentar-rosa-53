-- Fix RLS policies syntax - meal planning tables
...
create policy "Users read/write own plan days"
  on public.meal_plan_days
  for all
  using (exists (
    select 1 from public.meal_plans p
    where p.id = plan_id and (p.user_id = auth.uid() or has_role(auth.uid(), 'admin'::app_role))
  ))
  with check (exists (
    select 1 from public.meal_plans p
    where p.id = plan_id and (p.user_id = auth.uid() or has_role(auth.uid(), 'admin'::app_role))
...
create policy "Users read/write own meals"
  on public.meals
  for all
  using (exists (
    select 1
    ...
    where d.id = day_id and (p.user_id = auth.uid() or has_role(auth.uid(), 'admin'::app_role))
  ))
  with check (exists (
    select 1
    from public.meal_plan_days d
...
create policy "Users read/write own meal items"
  ...
  using (exists (
    ...
    join public.meal_plans p on p.id = d.plan_id
    where m.id = meal_id and (p.user_id = auth.uid() or has_role(auth.uid(), 'admin'::app_role))
  ))
  with check (exists (
    select 1
    from public.meals m
